...
export class GICSRangeReader {
    ...
    async fetchHeader() {
        ...
        const blockCount = view.getUint16(offset, true);
        offset += 2;
        const itemCount = view.getUint32(offset, true);
        offset += 4;
        const temporalIndexOffset = view.getUint32(offset, true);
        offset += 8; // skip high bits
        const itemIndexOffset = view.getUint32(offset, true);
        offset += 8;
     ...
     */
    findBlockForTimestamp(index, timestamp) {
        if (index.length === 0)
            return null;
     ...
     */
    async fetchBlock(blockId, index, header) {
     ...
     */
    getDownloadStats(fullFileSize, blocksDownloaded, avgBlockSize = 50 * 1024) {
 ...
 */
export function createRangeReader(url, config) {
