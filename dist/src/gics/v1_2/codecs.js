...
export class Codecs {
    ...
    static encodeBitPack(values) {
        ...
        if (max === 0)
            bits = 1; // Minimum 1 bit for zeros? Or 0 bits if all zero?
        if (bits === 0)
            bits = 1;
    ...
    static decodeBitPack(data, count) {
        if (data.length === 0)
            return [];
        const bits = data[0];
        const result = [];
    ...
    static encodeDict(values, context) {
        if (values.length === 0)
            return new Uint8Array(0);
        const output = [];
    ...
    static decodeDict(data, context) {
        if (data.length === 0)
            return [];
