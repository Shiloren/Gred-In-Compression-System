...
async function runBench() {
    ...
    // Warmup
    {
        ...
        for (const item of items.slice(0, 1000))
            await enc.addSnapshot({ timestamp: item.timestamp, items: new Map([[1, { price: item.value, quantity: 1 }]]) });
        await enc.flush();
    }
    ...
    const probes = telB.blocks.filter((b) => b.stream_id === 20 && (b.flags & 16)).length; // HEALTH_QUAR
