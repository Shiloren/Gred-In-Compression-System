...
import { generateTrendInt, Dataset } from './scripts/datasets.js'; // Assuming scripts is in bench/scripts
...
async function runEnc(ds: Dataset, env: Record<string, string> = {}): Promise<any> {
    // Set Env
    ...
    try {
        // Cold Run
        await measureSplit(async () => new GICSv2Encoder(), async (writer: any) => {
        ...
        for (let i = 0; i < runs; i++) {
            const m = await measureSplit(async () => new GICSv2Encoder(), async (writer: any) => {
                writerRef = writer;
                for (const row of ds.data) {
                    const itemMap = new Map();
                    itemMap.set(1, { price: row.v, quantity: 1 });
            ...
            output = m.result;
            if (writerRef && writerRef.getTelemetry) telemetry = writerRef.getTelemetry();
        }
...
async function runFamilyE() {
    ...
    // Good: Linear Trend
    for (let i = 0; i < 5000; i++) good1.push({ t: i * 10, v: i });

    // Noise: Random (Deterministic)
    const rng = new SeededRNG(999);
    for (let i = 0; i < 5000; i++) noise.push({ t: 50000 + i * 10, v: rng.next() * 1000000 });

    // Good: Return to Linear
    for (let i = 0; i < 20000; i++) good2.push({ t: 100000 + i * 10, v: 5000 + i });
    ...
    const qarBlocks = blocks.filter((b: any) => (b.flags & 16)).length;
    const startBlocks = blocks.filter((b: any) => (b.flags & 2)).length; // ANOMALY_START
    const endBlocks = blocks.filter((b: any) => (b.flags & 8)).length;   // ANOMALY_END
    ...
    // Verify
    if (startBlocks < 1) chmFailures.push("Family E: No Anomaly Start detected in Noise section");
    if (endBlocks < 1) chmFailures.push("Family E: No Anomaly End detected (No Recovery)");
}
...
async function runFamilyF() {
    ...
    for (let i = 0; i < 50000; i++) noise.push({ t: i * 10, v: rng.next() * 10000000 });
...
async function runDeterminismCheck() {
    ...
    const readSidecar = (name: string) => JSON.parse(fs.readFileSync(path.join(process.cwd(), name), 'utf-8'));
...
async function runCorrectnessCheck() {
    ...
    if (decoded.length !== ds.data.length) {
        ...
        for (let i = 0; i < decoded.length; i++) {
            ...
            if (decoded[i].timestamp !== ds.data[i].t) errs++;
            ...
            if (!item || item.price !== ds.data[i].v) errs++;
            ...
            if (errs <= 5 && (decoded[i].timestamp !== ds.data[i].t || (!item || item.price !== ds.data[i].v))) {
                ...
                if (decoded[i].items.size > 0) {
                    const k1 = decoded[i].items.keys().next().value as number;
        ...
        if (errs > 0) chmFailures.push(`Correctness: ${errs} Data Mismatches`);
        else console.log("Roundtrip: PASS");
    }
}
