...
function main() {
    ...
    if (files.length === 0) { console.log('No sensitive results.'); return; }
    ...
    for (const fam of families) {
        ...
        const familyRows = data.filter((r: any) => r.family === fam);
        if (familyRows.length === 0) continue;

        lines.push('| Variant | Mode | CtxID | AppMode | Ratio (x) | p50 (ms) | p90 (ms) | Output (Bytes) | Δ Bytes (OFF-ON) |');
        lines.push('|---|---|---|---|---|---|---|---|---|');

        const variants = Array.from(new Set(familyRows.map((r: any) => r.variant.replace(/_(OFF|ON)$/, '')))) as string[];

        for (const base of variants) {
            const off = familyRows.find((r: any) => r.variant === `${base}_OFF`);
            const on = familyRows.find((r: any) => r.variant === `${base}_ON`);

            if (off) lines.push(formatRow(base, 'OFF', off));
            if (on) lines.push(formatRow(base, 'ON', on));
...
function formatRow(base: string, mode: string, r: any): string {
