...
export async function measure<T>(fn: () => Promise<T> | T): Promise<{ result: T; metrics: Metrics }> {
...
export async function measureSplit<T>(
    setupFn: () => Promise<any> | any,
    actionFn: (context: any) => Promise<T> | T
...
export function stats(values: number[]) {
    if (values.length === 0) return { median: 0, p95: 0, p99: 0, min: 0, max: 0 };
